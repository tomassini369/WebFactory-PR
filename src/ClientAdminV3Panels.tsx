...
export function AnalyticsPanel({commerce,lang}:{commerce:{orders:Transaction[];bookings:Transaction[]};lang:Language}){
  ...
  return <><PortalPanel className="ca-panel"><header><h2>{es?'Analítica':'Analytics'}</h2><p>{es?'Vista consolidada de ventas, reservaciones y rendimiento comercial registrado por WebFactory.':'Consolidated view of sales, bookings and business performance recorded by WebFactory.'}</p></header><div className="ca-v3-metrics"><article><small>{es?'INGRESOS REGISTRADOS':'RECORDED REVENUE'}</small><strong>{money(revenue)}</strong></article><article><small>{es?'TRANSACCIONES PAGADAS':'PAID TRANSACTIONS'}</small><strong>{paid.length}</strong></article><article><small>{es?'TICKET PROMEDIO':'AVERAGE TICKET'}</small><strong>{money(average)}</strong></article><article><small>{es?'LINKS DE PAGO':'PAYMENT LINK SALES'}</small><strong>{paymentLinks}</strong></article></div></PortalPanel><PortalPanel className="ca-panel"><header><h2>{es?'Ingresos por canal':'Revenue by channel'}</h2><p>{es?'Separa ventas directas de ingresos por reservaciones.':'Separate direct sales from booking revenue.'}</p></header><div className="ca-v3-metrics"><article><small>{es?'ÓRDENES':'ORDERS'}</small><strong>{money(orderRevenue)}</strong></article><article><small>{es?'RESERVACIONES':'BOOKINGS'}</small><strong>{money(bookingRevenue)}</strong></article></div></PortalPanel><PortalPanel className="ca-panel"><header><h2>{es?'Más vendidos':'Top sellers'}</h2><p>{es?'Productos y servicios con mayor ingreso dentro del historial cargado.':'Products and services with the highest revenue in the loaded history.'}</p></header><div className="ca-v3-table">{topItems.length===0?<p>{es?'Todavía no hay datos suficientes.':'Not enough data yet.'}</p>:topItems.map(item=><article key={item.name}><div><strong>{item.name}</strong></div><div><b>{item.quantity}</b><span>{es?'Unidades':'Units'}</span></div><div><b>{money(item.revenue)}</b><span>{es?'Ingresos':'Revenue'}</span></div></article>)}</div></PortalPanel></>
}
...
export function InventoryStatusPanel({site,lang}:{site:any;lang:Language}){
  ...
  return <PortalPanel className="ca-panel"><header><h2>{es?'Inventario':'Inventory'}</h2><p>{es?'Estado del inventario rastreado en todo WebFactory.':'Inventory status tracked across WebFactory.'}</p></header><div className="ca-v3-metrics"><article><small>{es?'RASTREADOS':'TRACKED'}</small><strong>{tracked.length}</strong></article><article><small>{es?'BAJO INVENTARIO':'LOW STOCK'}</small><strong>{low.length}</strong></article><article><small>{es?'AGOTADOS':'SOLD OUT'}</small><strong>{out.length}</strong></article></div>{low.length>0&&<div className="ca-v3-table">{low.map((item:any)=><article key={item.id}><div><strong>{item.nameEn||item.name||item.nameEs}</strong><span>SKU {item.sku||'—'}</span></div><div><b>{Number(item.inventory??0)}</b><span>{es?'Disponibles':'On hand'}</span></div><div><b>{Number(item.lowStockThreshold??0)}</b><span>{es?'Alerta desde':'Alert at'}</span></div><div><b>{item.allowBackorder?(es?'Sí':'Yes'):(es?'No':'No')}</b><span>Backorder</span></div></article>)}</div>}</PortalPanel>
}
...
export function InventoryMovementsPanel({siteId,lang}:{siteId:string;lang:Language}){
  ...
  return <PortalPanel className="ca-panel"><header><h2>{es?'Movimientos de inventario':'Inventory movements'}</h2><p>{es?'Historial reciente de ventas, reembolsos y ajustes que cambiaron existencias.':'Recent sales, refunds and adjustments that changed stock.'}</p></header><div className="ca-transactions">{rows.length===0?<p>{es?'No hay movimientos todavía.':'No inventory movements yet.'}</p>:rows.map(row=><article key={row.movementId}><div><small>{String(row.reason||'adjustment').toUpperCase()}</small><strong>{row.itemId}</strong><span>{row.referenceId||row.movementId}</span></div><div><b>{Number(row.quantityDelta)>0?'+':''}{row.quantityDelta}</b><time>{new Date(row.createdAt).toLocaleString(es?'es-PR':'en-US')}</time></div></article>)}</div></PortalPanel>
}
...
export function MemberRolesPanel({site,setSite,lang,onSave,busy,membership}:{site:any;setSite:(site:any)=>void;lang:Language;onSave:(section:string,value:any)=>void;busy:boolean;membership:any}){
  ...
  return <PortalPanel className="ca-panel"><header><h2>{es?'Roles y acceso al portal':'Portal roles & access'}</h2><p>{es?'Invita miembros, asigna permisos y revoca acceso sin borrar sus datos del negocio.':'Invite members, assign permissions and revoke access without deleting business data.'}</p></header>{error&&<div className="ca-error">{error}</div>}{message&&<div className="ca-success">{message}</div>}{ownerOnly&&<div className="ca-grid"><label>Email<input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="employee@example.com"/></label><label>{es?'Rol':'Role'}<select value={role} onChange={e=>setRole(e.target.value)}><option value="manager">Manager</option><option value="employee">Employee</option><option value="cashier">Cashier</option></select></label><div className="ca-inline-action"><button className="ca-save" disabled={accessBusy==='invite'||!email.trim()} onClick={invite}>{accessBusy==='invite'?(es?'Enviando…':'Sending…'):(es?'Invitar miembro':'Invite member')}</button></div></div>}<div className="ca-v3-table">{members.map((member,index)=><article key={member.email}><div><strong>{member.email}</strong><span>{member.role}</span></div><div>{member.role==='owner'?<b>Owner</b>:<select disabled={!ownerOnly||busy||Boolean(accessBusy)} value={member.role||'employee'} onChange={e=>update(index,e.target.value)}><option value="manager">Manager</option><option value="employee">Employee</option><option value="cashier">Cashier</option></select>}</div>{ownerOnly&&member.role!=='owner'&&<div><button disabled={accessBusy===member.email} onClick={()=>revoke(member.email)}>{accessBusy===member.email?(es?'Revocando…':'Revoking…'):(es?'Revocar acceso':'Revoke access')}</button></div>}</article>)}</div>{ownerOnly&&<button className="ca-save" disabled={busy||Boolean(accessBusy)} onClick={()=>onSave('members',members)}>{busy?(es?'Guardando…':'Saving…'):(es?'Guardar cambios de roles':'Save role changes')}</button>}</PortalPanel>
}
...
export function IntegrationManagementPanel({site,setSite,lang,membership}:{site:any;setSite:(site:any)=>void;lang:Language;membership:any}){
  ...
  return <PortalPanel className="ca-panel"><header><h2>{es?'Servicios conectados':'Connected services'}</h2><p>{es?'Desconecta servicios externos sin eliminar tu página ni tu cuenta de WebFactory.':'Disconnect external services without deleting your WebFactory page or account.'}</p></header>{message&&<div className="ca-success">{message}</div>}{error&&<div className="ca-error">{error}</div>}<div className="ca-v3-table">
  ...
  </div><p className="ca-note">{es?'Desconectar un servicio elimina su vínculo con esta página. No elimina la cuenta que tengas directamente con el proveedor.':'Disconnecting removes the service link from this page. It does not delete your account with the external provider.'}</p></PortalPanel>
}
