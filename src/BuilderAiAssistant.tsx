...
export default function BuilderAiAssistant({state,setState,lang}:{state:any;setState:Dispatch<SetStateAction<any>>;lang:Language}){
  ...
  const generate=async()=>{
    ...
    try{
      ...
      if(!response.ok||!result.ok)throw new Error(result.message||'WebFactory AI could not generate a proposal.')
      setProposal(result.proposal)
    }catch(e){
      setError(e instanceof Error?e.message:(lang==='es'?'WebFactory AI no pudo completar la solicitud.':'WebFactory AI could not complete the request.'))
  ...
  return <section className={'wf-ai-assistant '+(open?'open':'')}>
    <button className="wf-ai-toggle" type="button" onClick={()=>setOpen(value=>!value)}>
      <span>✦</span><div><small>WEBFACTORY AI · CLAUDE</small><strong>{lang==='es'?'Construye o modifica con IA':'Build or modify with AI'}</strong></div><b>{open?'−':'+'}</b>
    </button>
    {open&&<div className="wf-ai-body">
      ...
      <div className="wf-ai-actions"><span>{prompt.length}/1400</span><button type="button" disabled={busy} onClick={generate}>{busy?(lang==='es'?'Claude está diseñando…':'Claude is designing…'):(lang==='es'?'Generar propuesta':'Generate proposal')} ✦</button></div>
      {error&&<div className="wf-ai-error">{error}</div>}
      {proposal&&<div className="wf-ai-proposal">
        <small>{lang==='es'?'PROPUESTA DE IA':'AI PROPOSAL'}</small>
