...
- [x] Removed `STRIPE_SECRET_KEY` from Netlify `deploy-preview` and `branch-deploy` contexts on 2026-09-24. Production live credentials remain unchanged. The runtime guard remains as defense in depth and also fails closed for unknown/dev-server hosts.\n- [ ] Configure an isolated Stripe test key and compatible test Connect account/webhook setup for Deploy Preview before running payment E2E tests. Deploy Preview currently has no Stripe secret key. Add an isolated Stripe test key plus compatible test Connect/webhook configuration before running payment E2E tests.
